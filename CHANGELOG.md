...
All notable changes to Vibe Coding OS are documented here.
...
- **`os3` CLI** — single entry point `bin/os3` replaces `make` targets and `os2.yaml`. Unified commands: `dispatch`, `approve`, `queue`, `status`, `pr-check`, `archive`, `dashboard`, `feedback`, `open`.
- **Host-OS architecture** — one host engine, many independent project repos under `projects/`. `os3 open <name>` injects host settings into any project session.
...
- **Local dashboard** — `os3 dashboard`, a build-free read-only kanban at `http://127.0.0.1:8787`.
- **OS-feedback loop** — `os3 feedback "..."` captures OS friction into `devos/os-feedback/INBOX.md`; reviewed at session start; converted to tickets in a periodic pass.
...
- **`drop-in` mode** — `scripts/dropin-init.sh` scaffolds the OS into any existing repo in ~5 minutes.

### Changed
- `make` / `Makefile` removed; all orchestration through `bin/os3`.
...
[v4.0.0]: https://github.com/theprdguy/Vibe-Coding-OS/releases/tag/v4.0.0
